...
class Iterator {
 public:
  Iterator(Hashtable<K, V, F> &table)
      : hashtable(table), position(0), current(NULL) {}

  Iterator(const Iterator &itr)
      : hashtable(itr.hashtable), position(0), current(NULL) {}
  ...
  bool hasNext() {
    ...
    if (position >= hashtable.capacity) return false;
...
class ExpiredIterator {
 public:
  ExpiredIterator(Hashtable<K, V, F> &table, timemilliseconds &base)
      : hashtable(table), position(0), current(NULL), basetime(base) {}

  ExpiredIterator(const ExpiredIterator &itr)
      : hashtable(itr.hashtable),
        position(0),
        current(NULL),
        basetime(itr.basetime) {}
  ...
  bool hasNext() {
    std::lock_guard<std::recursive_mutex> guard{hashtable.mutex};

    if (hashtable.periodSeconds == 0) return false;

    if (position >= hashtable.capacity) return false;

    if (current != NULL) {
      for (HashNode<K, V> *c = current->getNext(); c != NULL;
           c = c->getNext()) {
        if (isExpired(c)) {
          current = c;
          return true;
 ...
 public:
  Hashtable()
      : timerId(NULL),
        ...
        threshold(defaultCapacity * defaultLoadFactor),
        periodSeconds(0) {
    initTable();
  }

  Hashtable(int initCapability)
      : timerId(NULL),
        m_size(0),
        capacity(initCapability),
        loadFactor(defaultLoadFactor),
        threshold(initCapability * defaultLoadFactor),
        periodSeconds(0) {
    initTable();
  }

  Hashtable(int initCapability, float factor, int p = 0,
            void (*func)(K &) = NULL)
      : timerId(NULL),
        m_size(0),
        capacity(initCapability),
        loadFactor(factor),
        threshold(initCapability * factor),
        periodSeconds(p),
        expiredFunc(func) {
    initTable();
    if (periodSeconds == 0)
      ...
      ;
      timerId = Timer::getInstance().create(periodSeconds, periodSeconds,
                                            expire<K, V, F>, this);
    }
  }
  ...
  Iterator<K, V, F> keys() { return Iterator<K, V, F>(*this); }
 ...
 private:
  ...
  void rehash(const size_t newCapacity) {
    ...
    for (int j = 0; j < capacity; j++) {
      ...
      while (entry != NULL) {
        putEntryInternal(newTable, newCapacity, entry->getKey(),
                         entry->getValue(), entry->getTime());
        delete entry;
        entry = entry->getNext();
      }
    }
  ...
  bool putEntryInternal(HashNode<K, V> **targetTable, const size_t &size,
                        const K &key, const V &val,
                        const timemilliseconds &time) {
    HashNode<K, V> *prev = NULL;
